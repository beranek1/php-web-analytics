...
$ttlispsr = get_one_row("SELECT COUNT(*) FROM isps;", $b1_analytics_db);
$total_isps = $ttlispsr[0];
$mstrqstsr = get_rows_array("SELECT `visitor_country`, COUNT(*) FROM requests GROUP BY `visitor_country` ORDER BY COUNT(*) DESC;", $b1_analytics_db);
$top_countries = array();
...
$tpispsr = get_rows_array("SELECT `isp_id`, COUNT(*) FROM networks GROUP BY `isp_id` ORDER BY COUNT(*) DESC;", $b1_analytics_db);
$top_isps = array();
foreach($tpispsr as $isp) {
    $top_isps[$isp[0]] = $isp[1];
}
$tpurir = get_rows_array("SELECT `uri`, COUNT(*) FROM requests GROUP BY `uri` ORDER BY COUNT(*) DESC;", $b1_analytics_db);
$top_uris = array();
...
<title>beranek1 analytics</title>
<style>
body {
...
<footer>
    Powered by beranek1 analytics
    <a href="https://github.com/beranek1">Powered by beranek1 analytics</a>
</footer>
</html>
<?php
