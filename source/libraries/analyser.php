...
class web_analyser {
    ...
    function get_tops($table, $field, $start = null, $end = null, $timefield = "time") {
        $tops = array();
        $rows = null;
        if($start == null && $end == null) {
            $rows = $this->$db_manager->get_rows_array("SELECT `".$field."`, COUNT(*) FROM ".$table." GROUP BY `".$field."` ORDER BY COUNT(*) DESC;");
        } else if($start != null && $end == null) {
            $rows = $this->$db_manager->get_rows_array("SELECT `".$field."`, COUNT(*) FROM ".$table." WHERE ".$timefield." BETWEEN DATE_ADD(NOW(),INTERVAL ".$start.") AND NOW() GROUP BY `".$field."` ORDER BY COUNT(*) DESC;");
        } else {
            $rows = $this->$db_manager->get_rows_array("SELECT `".$field."`, COUNT(*) FROM ".$table." WHERE ".$timefield." BETWEEN DATE_ADD(NOW(),INTERVAL ".$start.") AND DATE_ADD(NOW(),INTERVAL ".$end.") GROUP BY `".$field."` ORDER BY COUNT(*) DESC;");
        }
        foreach($rows as $row) {
            $tops[$row[0]] = $row[1];
        }
    ...
    function get_total_count($table, $start = null, $end = null, $timefield = "time") {
        $row = null;
        if($start == null && $end == null) {
            $row = $this->$db_manager->get_one_row("SELECT COUNT(*) FROM ".$table.";");
        } else if($start != null && $end == null) {
            $row = $this->$db_manager->get_one_row("SELECT COUNT(*) FROM ".$table." WHERE ".$timefield." BETWEEN DATE_ADD(NOW(),INTERVAL ".$start.") AND NOW();");
        } else {
            $row = $this->$db_manager->get_one_row("SELECT COUNT(*) FROM ".$table." WHERE ".$timefield." BETWEEN DATE_ADD(NOW(),INTERVAL ".$start.") AND DATE_ADD(NOW(),INTERVAL ".$end.");");
        }
        return $row[0];
    }
    function analyse_countries($table, $field, $start = null, $end = null, $timefield = "time") {
        $total_countries = 0;
        $rows = null;
        if($start == null && $end == null) {
            $rows = $this->$db_manager->get_rows_array("SELECT `".$field."`, COUNT(*) FROM ".$table." GROUP BY `".$field."` ORDER BY COUNT(*) DESC;");
    ...
    function analyse_time($time = null) {
        $timespan = null;
        ...
        $total_requests = $this->get_total_count("requests", $start, $end);
        $total_visitors = $this->get_total_count("browsers", $start, $end);
        $total_isps = $this->get_total_count("isps", $start, $end);
        $total_networks = $this->get_total_count("networks", $start, $end);
        $total_useragents = $this->get_total_count("agents", $start, $end);
        $top_requests = $this->analyse_countries("requests", "visitor_country", $start, $end);
        $top_visitors = $this->analyse_countries("browsers", "country", $start, $end);
        $top_requests["uris"] = $this->get_tops("requests", "uri", $start, $end);
    ...
    function __construct($db_manager) {
        ...
        $this->statistics["alltime"] = analyse_time();
    }
}
?>
